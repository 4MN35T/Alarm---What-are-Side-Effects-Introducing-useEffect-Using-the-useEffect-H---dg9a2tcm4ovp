        ...
        <Fab color="primary" aria-label="add" onClick={props.onOpen} id="add-new">
          ...
          <div id="alarms">
            {props.alarms.length === 0 ? "No alarms found" : props.alarms.map((alarm, index) =>(
              <Alarm key={alarm.label} label={alarm.label} alarmTime={alarm.alarmTime} onDelete={()=> props.onDelete(index)} time={props.time} onEdit={(label1, alarmTime1, index)} index={index} onSnooze={() => props.onSnooze(index)}/>
            ))}
          </div>
        </>
    )
